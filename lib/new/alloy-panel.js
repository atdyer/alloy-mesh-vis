...
export default function alloy_panel () {
    ...
    panel.cli = function (cli) {
        ...
        cli.on_list(view_cli.message);
        cli.on_model(on_model_set);
        cli.on_ready(on_cli_ready);
        cli.on_result(view_cli.message);
    ...
    function on_model_set (event) {

        // Show the main view
        show_main_view();

        // Print the message to CLI
        if (view_cli) view_cli.message('Model set: ' + event.model);
