...
export default function instance_view (selection) {
    ...
    function view (selection) {
        ...
        if (!div) {
            ...
            // Add the properties pane
            properties_div = div.append('div')
                .attr('class', 'panel-item')
                .style('display', 'none');
            properties_view = properties_component();
            ...
            // Connect the properties view to the mesh view
            properties_view.on_property_styled(mesh_view.style_property);
    ...
    view.on_collapse = function (cb) {
        collapse_callback = cb;
    };

    view.on_expand = function (cb) {
        expand_callback = cb;
    };
    ...
    function collapse () {
        ...
        if (properties_div) {
            ...
            if (collapse_callback) collapse_callback();
            if (header) {
                header.icons().filter('.icon-fold')
                    .classed('icon-fold', false)
                    .classed('icon-unfold', true);
            }
    ...
    function expand () {
        ...
        if (properties_div) {
            expanded = true;
            properties_div.style('display', null);
            if (expand_callback) expand_callback();
            if (header) {
                header.icons().filter('.icon-unfold')
                    .classed('icon-fold', true)
                    .classed('icon-unfold', false);
            }
        }
    ...
    function display_mesh_solution (solution) {
        ...
        if (properties_div && properties_view) {

            properties_view(properties_div, solution);
